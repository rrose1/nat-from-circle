...
module _ {X : Set l1} {P : X → Set l2} {m1 m2 : (x : X) → P x} where
  postulate
...
module _ {X : Set l1} {Y : Set l2} (e : X ≃ Y) where
  ...
  pi-eqv-1' : {P : Y → Set l3} → ((x : X) → P (f e x)) ≃ ((y : Y) → P y)
...
module _ {X : Set l1} {Y : Set l2} (ϕ : is-contr Y) where
  ×unitr : X × Y ≃ X
  f ×unitr = fst
  g ×unitr x = x , fst ϕ
...
module _ {X : Set l1} {Y : Set l2} (P : X ⊎ Y → Set l3) where
  open _≃_
...
module _ {X : Set l1} {P : X → Set l2} {Q : {x : X} → (p : P x) → Set l3}
